...
/**
 * Hand-rolled in-memory [Settings] for core:storage's own tests.
 *
 * The canonical, shared copy lives in `core:testing`
 * ([com.m2f.template.core.testing.InMemorySettings]) for every other module to consume. core:storage
 * keeps a local copy because core:testing transitively depends on core:storage (core:testing →
 * core:sdk → core:storage), so depending on core:testing from here would create a project cycle.
 */
