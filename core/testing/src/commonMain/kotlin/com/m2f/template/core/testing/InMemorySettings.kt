...
/**
 * Hand-rolled in-memory [Settings] for tests. Avoids the `multiplatform-settings-test`
 * artifact (not published for every KMP target) and the unavailable `MapSettings`.
 *
 * Lives in core:testing's main source set so any module can consume it from its own tests.
 */
