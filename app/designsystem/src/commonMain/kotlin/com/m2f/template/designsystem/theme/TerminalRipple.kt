...
/**
 * A custom ripple indication for the terminal design system.
 *
 ...
 */
class TerminalRippleIndication(
    private val bounded: Boolean = true,
    private val color: Color = Color.Unspecified,
) : IndicationNodeFactory {

    override fun create(interactionSource: InteractionSource): DelegatableNode =
        TerminalRippleNode(interactionSource, bounded, color)

    override fun equals(other: Any?): Boolean {
        if (this === other) return true
        if (other !is TerminalRippleIndication) return false
...
private class TerminalRippleNode(
...
/**
 * Creates and remembers a [TerminalRippleIndication] configured for the current theme.
 *
 * @param bounded Whether the ripple clips to component bounds. Default `true`.
 * @param color Ripple color. If [Color.Unspecified], uses [TerminalTheme.colors.text] at 0.12f alpha.
 * @return A themed [TerminalRippleIndication] instance.
 */
@Composable
fun rememberTerminalRipple(
    bounded: Boolean = true,
    color: Color = Color.Unspecified,
): TerminalRippleIndication {
    val resolvedColor = if (color == Color.Unspecified) {
        TerminalTheme.colors.text.copy(alpha = 0.12f)
    } else {
        color
    }
    return remember(bounded, resolvedColor) { TerminalRippleIndication(bounded, resolvedColor) }
}
